...
pkgs.testers.runNixOSTest {
  ...
  testScript =
    ...
    ''
      ...
      machine.crash()
      machine.wait_for_unit("multi-user.target")
      machine.succeed("test \"$(findmnt -n -o UUID /)\" = \"$(blkid -s UUID -o value /dev/mapper/aurora_test-root)\"")
      machine.succeed("swapon --show=NAME --noheadings | xargs -n1 readlink -f | grep -Fx \"$(readlink -f \"$(blkid -U ${swapUuid})\")\"")
